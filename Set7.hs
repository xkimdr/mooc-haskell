...
data State = Start | Error | Finished | Started | E | EF | ES | TBM | TBB
  deriving (Eq, Show)

step :: State -> Event -> State
step Start AddEggs = E
step E AddFlour = EF
step E AddSugar = ES
step EF AddSugar = TBM
step ES AddFlour = TBM
step TBM Mix = TBB
step TBB Bake = Finished
...
passwordAllowed p (ContainsSome s) = foldr (\x acc -> acc || elem x p) False s
passwordAllowed p (DoesNotContain s) = foldr (\x acc -> acc && notElem x p) True s
