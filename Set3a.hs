...
capitalize xs = unwords (map (\ys -> toUpper (head ys) : tail ys) (words xs))
...
joinToLength n xs = [x | i <- xs, j <- xs, let x = i ++ j, length x == n]
...
sumRights xs = sum (map getRight xs)
  where
    getRight (Left _) = 0
    getRight (Right a) = a
...
interpreter commands = p commands 0 0 []
  where
    p [] x y ys = ys
    p ("up" : xs) x y ys = p xs x (y + 1) ys
    p ("down" : xs) x y ys = p xs x (y - 1) ys
    p ("right" : xs) x y ys = p xs (x + 1) y ys
    p ("left" : xs) x y ys = p xs (x - 1) y ys
    p ("printX" : xs) x y ys = p xs x y (ys ++ [show x])
    p ("printY" : xs) x y ys = p xs x y (ys ++ [show y])
