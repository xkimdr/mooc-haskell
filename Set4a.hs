...
longest' :: Ord a => Int -> [a] -> [[a]] -> [a]
longest' n rs [] = rs
longest' n rs (x : xs)
  | (l == n && head rs < head x) || n > l = longest' n rs xs
  | otherwise = longest' l x xs
  where
    l = length x
...
incrementKey _ [] = []
incrementKey a ((x, y) : xs) = (x, p) : incrementKey a xs
  where
    p = if a == x then y + 1 else y
...
transfer from to amount bank = case (Map.lookup from bank, Map.lookup to bank) of
  ...
  (_, Nothing) -> bank
  (Just fx, Just tx)
    | amount < 0 || fx < amount -> bank
    | otherwise -> Map.insert from (fx - amount) (Map.insert to (tx + amount) bank)
...
maxIndex arr = maxIndex' i (arr ! i) (Data.Array.assocs arr)
  where
    i = head (Data.Array.indices arr)

maxIndex' i e [] = i
maxIndex' i e ((x, y) : xs) = maxIndex' a b xs
  where
    (a, b) = if e > y then (i, e) else (x, y)