...
cull val (Node a t1 t2)
  | a == val = Empty
  | valAtRoot t1 == Just val = Node a Empty (cull val t2)
  | valAtRoot t2 == Just val = Node a (cull val t1) Empty
...
isOrdered (Node a t1 Empty) = valAtRoot t1 < Just a && isOrdered t1
isOrdered (Node a Empty t2) = valAtRoot t2 > Just a && isOrdered t2
isOrdered (Node a t1 t2) = valAtRoot t1 < Just a && valAtRoot t2 > Just a && isOrdered t1 && isOrdered t2
...
set (StepL : xs) val (Node a t1 t2) = Node a (set xs val t1) t2
set (StepR : xs) val (Node a t1 t2) = Node a t1 (set xs val t2)
set _ _ Empty = Empty
...
search val tree = search' val tree []
  where
    ...
    search' val (Node x t1 t2) xs
      | val == x = Just xs
      | search' val t1 (xs ++ [StepL]) == Nothing = search' val t2 (xs ++ [StepR])
      | otherwise = search' val t1 (xs ++ [StepL])
